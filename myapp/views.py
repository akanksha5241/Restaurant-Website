...
from .models import User, Cart, MyOrder, DeliveryPerson  # ensure all these are imported correctly
...
def checkout(request):
    ...
    if request.method == 'POST':
        ...
        for item in cart:
            menu_item = item.food_item  # Menu object

            MyOrder.objects.create(
				quantity=item.quantity,
				ordered_quantity=item.quantity,  # ✅ Add this line
				fname=request.POST['fname'],
				lname=request.POST['lname'],
				email=request.POST['email'],
				...
				state=request.POST['state'],
				pincode=request.POST['pincode'],
				food_item_name=menu_item.name,
				price=str(menu_item.price1),
...
def forgot_password(request):
    if request.method == "POST":
        try:
            ...
            return render(request, 'forget_password/otp.html', {'email': email, 'otp': otp, 'msg': msg})
        except:
            msg = "Email Not Registered"
            return render(request, 'forget_password/forgot_password.html', {'msg': msg})
    else:
        return render(request, 'forget_password/forgot_password.html')
...
def verify_otp(request):
    ...
    if otp == uotp:
        return render(request, 'forget_password/new_password.html', {'email': email})
    else:
        msg = "Invalid OTP"
        return render(request, 'forget_password/otp.html', {'email': email, 'otp': otp, 'msg': msg})
...
def new_password(request):
    ...
    if np == cnp:
        ...
        return render(request, 'forget_password/new_password.html', {'email': email, 'msg': msg})
...
def inventory_list(request):
    ...
    low_stock_items = items.filter(quantity__lte=5)
...
def get_category(request):
    ...
    if dish_id:
        item = InventoryItem.objects.filter(id=dish_id).first()
        if item:
            return JsonResponse({'category': item.category.name})
...
def get_dishes(request):
    ...
    if category_id:
        dishes = Menu.objects.filter(category_id=category_id).values('id', 'name')
